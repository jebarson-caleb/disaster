...
When standard SMTP recovery is configured, password-reset requests return the same response whether or not an active account exists. Raw reset tokens are sent only to the account email, stored only as SHA-256 hashes, expire after 30 minutes by default, and are single use. Completing recovery revokes every active session and clears lockout state. Failed email delivery immediately invalidates the generated token. Production readiness rejects partial SMTP configuration, insecure public reset URLs, and invalid token lifetimes.

After password authentication, server-side role checks select and restrict the user’s workspace. The client cannot switch a live account to another role, and facility users remain bound to their assigned operational record.
...
Configured outbound public warnings are sent only to an approved HTTPS webhook as a minimal CAP-inspired payload without user or sender contact data. Requests carry a stable alert idempotency key and an HMAC-SHA256 signature. Partial, insecure, or weak webhook configuration fails production readiness; delivery failures do not suppress the in-app warning and create a sanitized audit outcome. Attempt counts, bounded provider status codes, and attempt timestamps are retained with the alert so authorized command roles can retry safely without changing its idempotency key.
...
- Self-service password recovery is implemented but remains unavailable until the operator supplies an approved SMTP service, verified sender, public application URL, and delivery monitoring. Email ownership verification, phishing-resistant WebAuthn/passkeys, and external identity-provider federation require additional implementation or an approved identity service.
